...
# [ADDED][COPY-PASTE BLOCK 1 START] 질문 성격 기반 rerank 유틸
...
# 800자 넘어가는 청크 단위 몇개 있는지 확인용 코드
# 아래 코드 돌렸을 때 총 104개의 파일이 800자 초과한다고 나옴.
import os

def check_faq_length(path="./faq_output_md", limit=800):
    """
    지정된 폴더 내 마크다운 파일 중 글자 수가 limit을 초과하는 파일을 찾습니다.
    """
    ...
    print(f"🔍 {path} 폴더 내 {limit}자 초과 문서 검사 시작...\n")
        ...
        if filename.endswith(".md"):
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                if content_len > limit:
                    over_limit_files.append((filename, content_len))

    # 결과 출력
    if over_limit_files:
        print(f"⚠️ 총 {len(over_limit_files)}개의 파일이 {limit}자를 초과합니다:")
        print("-" * 50)
        print(f"{'파일명':<30} | {'글자 수':<10}")
        print("-" * 50)
        # 글자 수 내림차순 정렬
        over_limit_files.sort(key=lambda x: x[1], reverse=True)
        for name, length in over_limit_files:
            print(f"{name:<30} | {length:<10}자")
    else:
        print(f"✅ 모든 파일이 {limit}자 이하입니다. 안전하게 인덱싱 가능합니다.")

# 실행
check_faq_length(path="./faq_output_md", limit=800)
...
client = OpenAI(
    base_url="http://10.84.116.20:43021/llm/gpt-oss-120b-dev2/v1",
    api_key="dummy"
)
MODEL = "/models"
